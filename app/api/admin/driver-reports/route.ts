...
export async function GET(req: Request) {
  try {
    ...
    if (!employeeCode) {
      return NextResponse.json({ error: "Missing employeeCode" }, { status: 400 });
    }
    ...
    const url =
      `${base}?action=driverReports&employeeCode=${encodeURIComponent(employeeCode)}` +
    ...
    return NextResponse.json({ error: e.message || "Failed to fetch driver reports" }, { status: 500 });
  }
}
